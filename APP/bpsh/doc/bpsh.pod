...
B<bpsh> B<-h> I<remoteEID> B<-l> I<localEID> [B<-c> I<command>] [B<-k> I<secretfile>]
...
Authenticate to a B<bpshd> that was started with a shared secret, sending the
secret contained in I<secretfile> (a single line; a trailing newline is
ignored) in the INIT.  The secret may also be supplied in the environment
variable B<BPSH_SECRET>; B<-k> takes precedence.  Optional.
...
Shared secret to authenticate with, used when B<-k> is not given.
